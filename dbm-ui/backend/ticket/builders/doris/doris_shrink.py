...
class DorisShrinkDetailSerializer(BigDataSingleClusterOpsDetailsSerializer):
    ...
    def validate(self, attrs):
        ...
        role_hash = {
            ...
            InstanceRole.DORIS_BACKEND_WARM: attrs["old_nodes"]["warm"],
        ...
        # hot，warm必须有一个角色，每个角色至少需要2个节点
        shrink_hot_hosts = {host["bk_host_id"] for host in role_hash[InstanceRole.DORIS_BACKEND_HOT]}
        shrink_warm_hosts = {host["bk_host_id"] for host in role_hash[InstanceRole.DORIS_BACKEND_WARM]}
        exist_hot_hosts = set(
            [
                storage.machine.bk_host_id
                for storage in exist_storages
        ...
        exist_warm_hosts = set(
            [
                storage.machine.bk_host_id
                for storage in exist_storages
                if storage.instance_role == InstanceRole.DORIS_BACKEND_WARM
            ]
        )
        hot_node_count = len(exist_hot_hosts - shrink_hot_hosts)
        warm_node_count = len(exist_warm_hosts - shrink_warm_hosts)
        ...
        if not (hot_node_count >= constants.DORIS_HOT_WARM_LIMIT or warm_node_count >= constants.DORIS_HOT_WARM_LIMIT):
            raise serializers.ValidationError(_("请保证部署的温/热节点的角色为2以上"))
        if not total_nodes:
        ...
        if not set(all_exist_hosts).issuperset(set(all_shrink_hosts)):
            raise serializers.ValidationError(_("缩容仅支持hot、warm和observer"))
