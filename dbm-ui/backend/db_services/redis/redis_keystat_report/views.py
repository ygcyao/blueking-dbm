...
class KeyStatReportDetailsViewSet(viewsets.SystemViewSet):
    ...
    def export_keystat_analysis(self, request, bk_biz_id):
        ...
        wb = ExcelHandler.serialize(key_stat_data, headers=headers, match_header=True, sheet_name="内存分析报告")
        ExcelHandler.add_sheet(
            wb=wb, sheet_name="大Key排行榜", data_dict__list=rank_data, headers=rank_headers, match_header=True
        )
